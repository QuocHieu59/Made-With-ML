...
def get_best_checkpoint() -> TorchCheckpoint:  # pragma: no cover, mlflow logic
    ...
    """
    #artifact_uri = mlflow.get_run(run_id).info.artifact_uri.replace("\\", "/")
    #parsed_uri = urlparse(artifact_uri)
    #artifact_dir = Path(parsed_uri.netloc + parsed_uri.path).as_posix() if parsed_uri.netloc else Path(parsed_uri.path).as_posix()
    #artifact_dir = urlparse(mlflow.get_run(run_id).info.artifact_uri.replace("\\", "/")).path  # get path from mlflow
    #print(mlflow.get_run(run_id).info.artifact_uri.replace("\\", "/"))
    #print(artifact_dir)
    ...
    results = Result.from_path(saved_path)
    #print(results)
    #print(results.best_checkpoints[0][0])
