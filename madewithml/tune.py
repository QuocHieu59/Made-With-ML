...
def tune_models(
    ...
    initial_params = json.loads('[{"train_loop_config": {"dropout_p": 0.5, "lr": 1e-4, "lr_factor": 0.8, "lr_patience": 3}}]')
    ...
    hyperparameters_fp = EFS_DIR / "hyperparameters.yaml"
    with open(hyperparameters_fp, "w") as yaml_file:
        yaml.dump(best_trial.config["train_loop_config"], yaml_file, default_flow_style=False)
